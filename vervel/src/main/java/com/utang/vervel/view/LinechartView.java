...
public class LinechartView extends View {
    ...
    public void setValuesL(long[] valuesL) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (type == null) {
            ...
            Rect rectX = new Rect();
            rectX.left = 150 +  getPaddingRight();
            rectX.top = mToppadding + getPaddingTop();
            rectX.right = getWidth() - getPaddingRight() - 50;
            rectX.bottom = getHeight() - getPaddingBottom() - 50;
            drawIng(canvas, null, valuesX, rectX);
        }
            ...
            rectX.right = getWidth() - getPaddingRight() - 50;
            rectX.bottom = getHeight() - getPaddingBottom() - 50;
            drawIng(canvas, null, valuesL, rectX);
        }

    }

    private void drawIng(Canvas canvas, String type, long[] values, Rect r) {
        ...
        for (int i = 0; i < 11; i++) {
            ...
            if (i == 10) {
                time = DateUtils.getDateToString(times[times.length - 1] * 1000);
            }
    ...
    private int[] getMinMaxValue(int[] values) {
        ...
        max = (int) (Math.ceil(max * 1.0f / 100.0f) * 100);
        min = (int) (Math.floor(min * 1.0f / 100.0f) * 100);
        if (max + min == 0)
    ...
    private long[] getMinMaxValue(long[] values) {
        ...
        min = (int) (Math.floor(min * 1.0f / 100.0f) * 100);
        if (max + min == 0)
            return new long[]{min, 0, max};
        else {
            if ((max + min) % 200 == 0)
                return new long[]{min, (min + max) / 2, max};
            else return new long[]{min, (min + max + 100) / 2, max + 100};
        }
